...
# --- 1. Model Loading ---
MODEL_PATH = '../../models/adult_model/mania_lgbm_model.pkl'
ENCODER_PATH = '../../models/adult_model/mania_label_encoder.pkl'

try:
    model = joblib.load(MODEL_PATH) 
    le = joblib.load(ENCODER_PATH) 
except FileNotFoundError:
    print(f"FATAL ERROR: Mania model files not found at {MODEL_PATH}.")
    exit()
...
def predict_mania_severity(raw_scores):
    """
    Accepts 5 raw ASRM scores and calculates the required 7 features.
    (5 Items + Total Raw Score + Prorated Score)
    """
    if len(raw_scores) != 5:
        raise ValueError("Input must contain exactly 5 scores for the ASRM Mania scale.")
...
if __name__ == '__main__':
    ...
    print("-" * 40)
    print(f"Scores: {scores_1} | Result: {predict_mania_severity(scores_1)}")
    print(f"Scores: {scores_2} | Result: {predict_mania_severity(scores_2)}")
    print("-" * 40)