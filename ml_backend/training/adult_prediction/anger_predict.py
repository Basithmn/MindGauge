...
# --- 1. Model Loading ---
MODEL_PATH = '../../models/adult_model/anger_lgbm_model.pkl'
ENCODER_PATH = '../../models/adult_model/anger_label_encoder.pkl'
...
def predict_anger_severity(raw_scores):
    """
    Accepts 5 raw PROMIS scores and calculates the required 7 features.
    (5 Items + Total Raw Score + Prorated Score)
    """
    if len(raw_scores) != 5:
        raise ValueError("Input must contain exactly 5 scores for the Adult Anger scale.")
...
if __name__ == '__main__':
    ...
    print(f"Scores: {scores_3} | Result: {predict_anger_severity(scores_3)}")
    print("-" * 40)