...
# --- 1. Model Loading ---
MODEL_PATH = '../../models/adult_model/anxiety_lgbm_model.pkl'
ENCODER_PATH = '../../models/adult_model/anxiety_label_encoder.pkl'

try:
    model = joblib.load(MODEL_PATH) 
    le = joblib.load(ENCODER_PATH) 
except FileNotFoundError:
    print(f"FATAL ERROR: Anxiety model files not found.")
    exit()
...
def predict_anxiety_severity(raw_scores):
    if len(raw_scores) != 7:
        raise ValueError("Input must contain exactly 7 scores.")
    
    tr = sum(raw_scores)
    ps = tr 
    
    # Assemble 9 features: 7 items + TR + PS
    input_data = list(raw_scores) + [tr, ps]
    
    feature_names = [sanitize_name(f'Q{i}') for i in range(1, 8)] + \
...
if __name__ == '__main__':
    ...
    scores_1 = [4, 5, 4, 5, 4, 5, 4] 
    scores_2 = [1, 1, 1, 2, 1, 1, 1]
