...
try:
    ...
    print(f"FATAL ERROR: Depression model files not found at {MODEL_PATH}.")
    exit()
...
def predict_depression_severity(raw_scores):
    """
    Accepts 14 raw PROMIS scores and calculates the required 16 features.
    (14 Items + Total Raw Score + Prorated Score)
    """
    if len(raw_scores) != 14:
        raise ValueError("Input must contain exactly 14 scores for the Children's Depression scale.")
        
    # Step 1: Calculate Total Raw Score (TR) and Prorated Score (PS)
    tr = sum(raw_scores)
    ps = tr
    
    # Step 2: Assemble the 16-Feature Input List
...
if __name__ == '__main__':
    ...
    scores_1 = [5, 4, 5, 5, 4, 5, 4, 5, 4, 5, 5, 4, 5, 5] 
    scores_2 = [1, 1, 1, 2, 1, 1, 1, 1, 1, 2, 1, 1, 1, 1]
    
    print("-" * 50)
    print("CHILDREN DEPRESSION (PROMIS-14) PREDICTOR")
    print("-" * 50)
