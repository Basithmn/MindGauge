...
# --- 1. Model Loading ---
MODEL_PATH = '../../models/children_model/substance_use_lgbm_model.pkl'
ENCODER_PATH = '../../models/children_model/substance_use_label_encoder.pkl'
...
def predict_substance_use_category(raw_scores):
    """
    Accepts 15 raw scores.
    Calculates 1 feature (TR).
    Total features: 15 items + 1 TR = 16 features total.
    """
    if len(raw_scores) != 15:
        raise ValueError("Input must contain exactly 15 scores.")
        
    # Step 1: Calculate Total Raw Score (TR) only
    tr = sum(raw_scores)
    
    # Step 2: Assemble the 16-Feature Input List (15 Items + TR)
